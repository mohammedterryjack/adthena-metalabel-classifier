...
from src.models.utils import ModelSettings, parallel_map
...
class TopicClassifier:
    def __init__(
        ...
        if isfile(self.settings[ModelSettings.MODEL_PATH.value]):
            ...
            self.__model = load(self.settings[ModelSettings.MODEL_PATH.value])
        else:
    ...
    def train(self, search_terms: List[str], categories: List[int]) -> None:
        ...
        dump(self.__model, self.settings[ModelSettings.MODEL_PATH.value])
